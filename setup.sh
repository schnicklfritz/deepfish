...
# HF download acceleration (Rust-based parallel downloader, up to 2x).
# Caveat: no resume support — partial downloads restart from zero.
# HF_TOKEN should be set as a pod env var if you have one (better rate limits).
...
$PIP install --quiet --no-cache-dir openai requests soundfile b2 hf_transfer
...
log "skipping Gradio webui :7860 (would OOM 24GB cards alongside api_server)"
log "  to enable later: uncomment the run_webui.py block in setup.sh,"
log "  AND rent a card with >32 GB VRAM (2x3090, A40, A100, etc)."
